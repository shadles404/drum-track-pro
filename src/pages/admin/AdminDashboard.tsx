...
import { Package, AlertTriangle, Clock, TrendingUp, CheckCircle } from 'lucide-react';
  ...
  return (
    <DashboardLayout title="Admin Dashboard" subtitle="Overview of all drum sales and returns">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-8">
        <StatCard title="Sales This Month" value={statsLoading ? '...' : stats?.totalSalesThisMonth || 0} icon={Package} variant="primary" className="stagger-1" />
        <StatCard title="All-Time Sales" value={statsLoading ? '...' : stats?.totalSalesAllTime || 0} icon={TrendingUp} variant="default" className="stagger-2" />
        <StatCard title="Pending Approval" value={statsLoading ? '...' : stats?.totalPendingApproval || 0} icon={Clock} variant="warning" className="stagger-3" />
        <StatCard title="Overdue Drums" value={statsLoading ? '...' : stats?.totalOverdue || 0} icon={AlertTriangle} variant="danger" className="stagger-4" />
      </div>
